...
#TODO: ver por que no funciona con algunas querys
...
def create_user(
    ...
    return user_repo.create_user(new_user=new_user)
...
def authenticate_user(
    ...
    user: UserValidation = user_repo.get_user_by_login(login=credentials.login)
    if not user:
...
def update_user_commission(
    ...
    user = user_repo.update_user_commission(request.id, request.commission)
    if not user:
...
def update_user_status(
    ...
    return user_repo.update_user_status(request.id, request.is_active)
...
#     user_repo: UserRepository = Depends(get_repository(UserRepository))
#     ):
#     res = user_repo.update_user_password(id, request.old_password, request.new_password)
#     if not res:
#         raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="User not found")
